...
export type AIConversationProps = {
  ...
  /** Use the light-on-dark scrollbar tint for panels that are always dark
   * regardless of site theme (e.g. the AI Demo section). */
  dark?: boolean;
};
...
const AIConversation = ({
  ...
  return (
    <div className={cn("relative min-h-0 w-full", className)}>
      <div
        className={cn("h-full overflow-y-auto overscroll-contain thin-scroll", dark && "thin-scroll-dark")}
        data-lenis-prevent
        onScroll={measure}
        ref={viewportRef}
      >
